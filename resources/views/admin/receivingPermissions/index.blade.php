...
@section('pagetitle') Revenues@endsection
...
<div class="container-fluid h-100">
  ...
  @if($add == "yes")
        ...
        <a href="{{ route('receiving_permission.add',['menuid' =>$id ]) }}" class="pull-right action add"> <i class="fa fa-plus"></i>
    ...
    @endif          
     ...
     <form action="{{ route('receiving_permission.search',['id'=>$id])}}" method="post" class="form-out">
         {{ csrf_field() }}
         <div class="row">
                   <div class="col-3">    
  ...
  ?>
    @foreach ($permissions as $item)
       ...
       <tr class="table-row">
          ...
          <td class="cell-row">@if(isset($item->VarAsset)){{$item->VarAsset->name}}@endif</td>
        ...
        @if($update == "yes")
            ...
            <a href="{{ route('receiving_permission.edit',['receiving_permissionid' =>$item->id,'menuid' =>$id ]) }}" class="edit"><i class="fa fa-edit"></i></a> 
      
        @endif
       @if($delete == "yes")
    
         <a href="{{ route('receiving_permission.destory',['receiving_permissionid' =>$item->id,'menuid'=>$id ]) }}" onclick="return confirm('هل أنت متأكد')" class="edit">
