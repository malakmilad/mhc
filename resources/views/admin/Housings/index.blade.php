...
@section('contentheader') 
 <section class="content-header">
	  <h1>
	    @lang('home.housing')
	    <small>	@lang('home.housing_desc') </small>
	  </h1>
	  <!-- <ol class="breadcrumb">
	    <li><a href="#"><i class="fa fa-dashboard"></i>@lang('home.control') </a>
      </li>
	    <li class="active">@lang('home.housing')</li>
	  </ol> -->
            ...
            <div class="col-xs-12">
              <div class="box box-info">
                <div class="box-header">
                  <h3 class="box-title"> @lang('home.housing')</h3>
                  <div class="box-tools">
                    ...
                    <a href="{{ route('housing.create',$menuid) }}" class="pull-right action add"> <i class="fa fa-plus"></i> Add </a>
                    @endif  
                  ...
                  <table class="display table-bordered" style="width:100%" id="report">
                   ...
                   @if($allHousings->count() > 0)
                    ...
                    <tr>
                      ...
                      <th class="text-center">@lang('home.phone')</th>
                    ...
                    <tbody>
                  @foreach($allHousings as $act)
                    <tr>
                      <td class="text-center">{{ $i }}</td>
                      <td class="text-center">{{ $act['id'] }}</td>
                      <td class="text-center">{{ $act['name'] }}</td>
                      <td class="text-center">{{ $act['email'] }}</td>
                      <td class="text-center">{{ $act['address'] }}</td>
                      <td class="text-center">{{ $act['phone'] }}</td>
                      <td class="text-center">{{ date('d/m/Y',strtotime($act['created_at'])) }}</td>
                      <td class="text-center">
                      @if($update == "yes")
                      <a href="{{ route('housing.edit',['Housing' => $act['id'] , 'menuid' => $menuid ] ) }}" class="action edit"><i class="fa fa-pencil"></i> </a>
                      @endif
                      @if($delete == "yes")
                      <a href="{{ route('housing.destory',['Housing' => $act['id'] , 'menuid' => $menuid ] ) }}" class="action delete"> <i class="fa fa-times"></i> </a>
                      @endif
                      </td>
                    </tr>
                    <?php $i= $i + 1;?>
