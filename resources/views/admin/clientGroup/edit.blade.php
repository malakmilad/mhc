     ...
     <div class="col-xs-12">
        <div class="box box-info">
            ...
            @if(count($errors) > 0)
                        ...
                        <div></div>
                    @endif
              <form action="{{ route('client_groups.update',$clientGroup['id']) }}" method="post">
                {{ csrf_field() }}
                <input type="hidden" name="menuid" value="{{ $menuid }}">
                <div class="form-group">
                   <lable>  @lang('home.name') </lable> <span style="color: red;">*</span>
                  <input type="text" class="form-control" name="name" value="{{ $clientGroup->name }}" required placeholder="@lang('home.name')">
                </div>
                <div class="form-group">
