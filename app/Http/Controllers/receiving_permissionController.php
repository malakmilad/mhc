...
use App\TimeTable;
class receiving_permissionController extends Controller
{
    //
    public function index($id)
    {
      $permissions=ReceivingPermissions::get();
      $menus=Menu::all();
      ...
      $type=1;
      return view('admin.receivingPermissions.index',compact('permissions','menus','id','type'));
    }
   
    public function search($id,Request $request)
    {
      $permissions=ReceivingPermissions::where('created_at', '>=', $request->date_from.' 00:00:00')->where('created_at', '<=', $request->date_to.' 00:00:00')->get();
      $menus=Menu::all();
      foreach($permissions as $item)
      {
          if($item->companyid!=0)
      ...
      $type=1;
      return view('admin.receivingPermissions.index',compact('permissions','menus','id','date_from','date_to','type'));
    }
    ...
    //  $accounts=array();
     /* foreach($clients as $account)
       ...
       {
           array_push($accounts,array("id"=>"2,".$account->id,"name"=>"الموردين  / ".$account->name));
       }
       ...
       foreach($generic_accounts as $account)
       {
        ...
        $statement = DB::select("show table status like 'receiving_permissions'");
        $next_id=$statement[0]->Auto_increment;
    //  $cost_centers_accounts=CostAccount::all();
      //$cost_centers=CostCenter::all();
      return view('admin.receivingPermissions.add',compact('menus','var_assets','menuid','accounts','next_id','users'));
  
     // return view('admin.receivingPermissions.add',compact('var_assets','menus','menuid','accounts','next_id','cost_centers','users'));
    }
            ...
            $operations = DB::table('time_tables')
                        ...
                        ->whereNotIn('id',function ($query) {
                            $query->select(DB::raw('orderid'))
                                  ->from('receiving_permissions');
                        })->where('company_id',$company)->where("added_value", '!=',NULL)
                        ->get();
            $orders_ids = array();
    ...
    public function store(Request $request)
    {
     ...
     //   if($type==3)
      ...
      /*  if($type==4)
        ...
        return redirect('receiving_permissions/'.$request->menuid);
    ...
    {
       $repermission=ReceivingPermissions::whereId($id)->first();
     $orders_ids=array();
      if($repermission->companyid!=0 )
        {
            ...
            $orders = DB::table('time_tables')
                        ->select('time_tables.*')          
                        ->where('company_id',$repermission->companyid)->where("added_value", '!=',NULL)
                        ->get(); 
      ...
      // $suppliers=Client::where("type",2)->get();
       ...
       foreach($companies as $account)
       {
        ...
        return view('admin.receivingPermissions.edit',compact('var_assets','repermission','menus','menuid','accounts','orders_ids'));
    ...
    public function edit(Request $request)
    {
        $this->validate(
            $request, 
            ['id'=>'required|unique:receiving_permissions,id,'.$request->repermissionid],
            ['id.unique' => 'هذا الكود موجود من قبل']
        );
        $permission=ReceivingPermissions::find($request->repermissionid);
        $permission->id=$request->id;
    ...
    {
        $deleted=ReceivingPermissions::whereId($id)->first()->delete();
        return redirect('receiving_permissions/'.$menuid);   
    }
    
    public function report($id)
    {
        
     $permissions=ReceivingPermissions::whereNotNull("orderid")->where(function ($query) {$query->where('clientid', '!=', 0)->orWhere('companyid', '!=', 0);})->get();
      $menus=Menu::all();
      foreach($permissions as $item)
      {
          if($item->companyid!=0)
      ...
      $clients=Client::where("type","!=",2)->get();
        ...
        $users=User::all();
      return view('admin.receivingPermissions.report',compact('permissions','menus','id','accounts','users'));
    ...
    public function report_filter($id,Request $request)
    {
     ...
     $permissions=ReceivingPermissions::whereNotNull("orderid")->where($conditions)->where(function ($query) {$query->where('clientid', '!=', 0)->orWhere('companyid', '!=', 0);})->get();
      $menus=Menu::all();
      foreach($permissions as $item)
      {
          if($item->companyid!=0)
      ...
      $clients=Client::where("type","!=",2)->get();
        ...
        $users=User::all();
      return view('admin.receivingPermissions.report',compact('permissions','menus','id','accounts','users','filter'));
